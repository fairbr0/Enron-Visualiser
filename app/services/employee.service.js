    ...
    return {
        ...
        execute: function() {
            EmployeeService = (function () {
                ...
                EmployeeService.prototype.searchEmployees = function (emailString) {
                    return this.http.get('http://' + this.url + '/employees?search=' + emailString + '&number=20')
                        .map(function (response) { return response.json(); });
                };
                EmployeeService.prototype.logError = function (err) {
                    console.error('There was an error: ' + err);
                };
                EmployeeService.prototype.getEmployee = function (id, start, end, period, amount) {
                    return this.http.get('http://' + this.url + '/employee-rates?id=' + id + '&start=' + start + '&end=' + end + '&time-period=' + period + '&period-amount=' + amount)
                        .map(function (response) { return response.json(); });
                };
                EmployeeService.prototype.getTimes = function () {
